    ...
    if type(project_config) == "table" and project_config.lsp_config then
      ...
      for _, path in ipairs(M.local_config_paths) do
        ...
        if p:exists() and p:is_file() then
          local ok, data = pcall(vim.secure.read, path)
...
-- Lua (using vim.lsp.config API for Neovim 0.11+)
vim.lsp.config("lua_ls", M.create_config({
  settings = {
    Lua = {
      runtime = {
        version = "LuaJIT",
      ...
      diagnostics = {
        -- Recognize vim and other Neovim globals
        globals = {
          "vim",
          "Config",
          "prequire",
