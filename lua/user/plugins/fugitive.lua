return function()
  ...
  function M.get_blame_cursor_info()
    ...
    return {
      file = Path.from(state.cwd):join(state.blame_file):tostring(),
  ...
  au.declare_group("fugitive_config", {}, {
    ...
    },
    {
      ...
      callback = function(ctx)
        ...
        -- Config.lib.mv_keymap stub - needs to be implemented
            ...
            if commit:find(":") then
              ...
              vim.cmd(
                fmt(
                  ...
                  commit,
                  vim.fn.fnameescape(
                    Path.from(vim.fn.FugitiveWorkTree()):join(path):tostring()
                  )
                )
              )
            else
